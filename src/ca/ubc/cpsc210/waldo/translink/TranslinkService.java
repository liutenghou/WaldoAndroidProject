...
public class TranslinkService {
	...
	private final static String APIKEY = "kQ5lLBHbc4xwnW0MwZkV"; 
		...
		try {
			...
			if (in != null)
				try {
					...
					System.out.println("Oh well, couldn't free resource. Don't worry about it for now");
				}
		}
	...
	private void parseBusEstimatesFromJSON(BusStop stop, String input) {
		...
		try {
			...
			if (obj != null) {
				...
				//Log.d("7 latlon", "the whole jsonArray obj: "+ obj);
				...
				for (int i = 0; i < obj.length(); i++) {
					...
					routeMapLocation = routeMapLocation.replace("\"", "");
					Log.d("6 latlon", "parseBusEstimatesFromJSON.routeMapLocation: " + routeMapLocation);
					...
					Log.d("7 latlon", "busRoute with URL: "+ busRoute.getRouteNumber());
				}
			}
